...
struct TracksBuilder {
    ...
    using IndMatch = std::pair<uint32_t, uint32_t>;
    using PairWiseMatches = std::map<IndexPair, std::vector<IndexPair>>;
    ...
    void Build(const PairWiseMatches& map_pair_wise_matches)
    {
        ...
        for (const auto& iter : map_pair_wise_matches) {
            ...
            for (const auto& cur_filtered_match : vec_FilteredMatches) {
                allFeatures.emplace(I, cur_filtered_match.first);
                allFeatures.emplace(J, cur_filtered_match.second);
            }
        }
        ...
        for (const auto& iter : map_pair_wise_matches) {
            ...
            for (const IndMatch& match : vec_FilteredMatches) {
                const indexedFeaturePair pairI(I, match.first);
                const indexedFeaturePair pairJ(J, match.second);
    ...
    bool Filter(size_t nLengthSupTo = 2)
    {
        ...
        std::map<uint32_t, std::set<uint32_t>>
            tracks;                               // {track_id, {image_id, image_id, ...}}
        std::set<uint32_t> problematic_track_id;  // {track_id, ...}

        // For each node retrieve its track id from the UF tree and add the node to the
        // track
    ...
    {
        std::set<uint32_t> parent_id(uf_tree.m_cc_parent.begin(),
